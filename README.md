...
- The ETL's final stage (`etl/07_build_serving.py`) emits a compact **serving layer** (`data/serving/*.parquet`, ~45 MB: `core_trade`, `signals`, `peer_groups`, `hs6_names`, `countries`). The API reads **only** that — done in **M4b** (✓). The ~1.5 GB raw/derived matrix never leaves the ETL stage. The serving artifact ships as a GitHub Release asset that the deploy downloads (wired in M7).
- **Memory-bounded reads (M7).** `core_trade` is 1.78M rows; loading it whole into pandas spikes RSS to ~700 MB and OOMs the 512 MB host. So the API never does that: the map/products path reads only the 5 columns it needs (`api/data/serving.py`), and `/bars`, `/trend`, `/insights_data` query the parquet **on demand with DuckDB** (`api/data_access.query_core`), pushing year/hs6/partner filters down so only the needed rows/columns are read. `core_trade.parquet` is written sorted by `(year, hs6)` in 50k-row groups so those queries skip most of the file. Result: the full dashboard runs in ~356 MB — fits the Free tier.
