...
| M7 | Hosting & deploy | ✅ done (branch `m7-hosting`) — **LIVE** | M3, M4b, M5, M6 | **Live: https://mapa-prilezitosti-z6ch.onrender.com** (FastAPI single service, Render **Free** tier, Frankfurt). Release-asset flow; full dashboard fits 512 MB via **DuckDB on-demand queries + repartitioned `core_trade`** (`api/data_access.query_core`, one connection, `memory_limit=200MB`); fixed UI hardcoded-localhost API base (same-origin), SPA-shadow root route, missing `python-dotenv`. `verify-deploy.command` 7/7 on live; survived 120 heavy `/bars`. Runbook `RUNBOOK.md`. **Deferred:** insights AI-field (set `ANTHROPIC_API_KEY`), M2 truth pass, optional Starter bump |
